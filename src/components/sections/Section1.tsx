...
.ifs-grid{display:grid;grid-template-columns:1fr 1fr;gap:clamp(32px,4vw,64px);align-items:start;}

/* ═══ LEFT: COPY ═══ */
.ifs-copy{display:flex;flex-direction:column;gap:clamp(18px,2vw,28px);padding-top:clamp(8px,1vw,20px);}
...
.ifs-headline{font-family:'Outfit',sans-serif;font-size:clamp(28px,3vw,48px);font-weight:800;line-height:1.08;letter-spacing:-0.02em;text-transform:uppercase;color:var(--navy);opacity:0;}
.ifs-hl-accent{color:var(--accent);}
.ifs-body{font-size:clamp(14px,1.05vw,17px);color:var(--muted);line-height:1.78;max-width:500px;opacity:0;}
...
/* ═══ RIGHT: CARD ═══ */
.ifs-card{background:#fff;border:1px solid rgba(11,60,93,0.06);border-radius:12px;padding:clamp(16px,1.8vw,24px) clamp(14px,1.5vw,22px) clamp(14px,1.4vw,20px);box-shadow:0 1px 3px rgba(11,60,93,0.04),0 4px 14px rgba(11,60,93,0.04),0 14px 40px rgba(11,60,93,0.05),0 28px 72px rgba(11,60,93,0.03);opacity:0;}
.ifs-card-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:clamp(8px,1vw,14px);padding-bottom:clamp(8px,0.8vw,12px);border-bottom:1px solid rgba(11,60,93,0.05);}
...
.ifs-card canvas{display:block;width:100%;height:clamp(300px,36vh,420px);border-radius:6px;}
.ifs-card-bottom{display:flex;gap:clamp(14px,1.6vw,24px);justify-content:center;padding-top:clamp(8px,0.8vw,12px);margin-top:clamp(8px,0.8vw,12px);border-top:1px solid rgba(11,60,93,0.04);opacity:0;}
...
@media(max-width:960px){.ifs-grid{grid-template-columns:1fr;}.ifs-card{max-width:640px;}}
@media(min-width:1800px){.ifs-wrap{max-width:1560px;padding:100px 110px;}.ifs-headline{font-size:54px;}}
@media(min-width:2400px){.ifs-wrap{max-width:1700px;}.ifs-headline{font-size:62px;}}
</style>
...
if(bgC){
  ...
  var bt=0;
  function bF(){bt++;bX.clearRect(0,0,bW,bH);
    for(var i=0;i<nd.length;i++){var n=nd[i];n.x+=n.vx+Math.sin(bt*0.0008+i)*0.00002;n.y+=n.vy+Math.cos(bt*0.001+i*0.7)*0.00002;if(n.x<-0.1)n.x=1.1;if(n.x>1.1)n.x=-0.1;if(n.y<-0.1)n.y=1.1;if(n.y>1.1)n.y=-0.1;}
    for(var i=0;i<nd.length;i++){for(var j=i+1;j<nd.length;j++){var dx=(nd[i].x-nd[j].x)*bW,dy=(nd[i].y-nd[j].y)*bH,d=Math.sqrt(dx*dx+dy*dy);if(d<150){bX.strokeStyle='rgba(11,60,93,'+(0.018*(1-d/150))+')';bX.lineWidth=0.5;bX.beginPath();bX.moveTo(nd[i].x*bW,nd[i].y*bH);bX.lineTo(nd[j].x*bW,nd[j].y*bH);bX.stroke();}}}
    for(var i=0;i<nd.length;i++){bX.beginPath();bX.arc(nd[i].x*bW,nd[i].y*bH,nd[i].r,0,Math.PI*2);bX.fillStyle='rgba(11,60,93,0.025)';bX.fill();}
...
/* ═══ GSAP ═══ */
if(!reduced){
  gsap.set('#ifsEye',{opacity:0,x:-20});
  gsap.set('#ifsHL',{opacity:0,y:20});
  gsap.set('#ifsBody',{opacity:0,y:18});
  gsap.set('#ifsExLabel',{opacity:0,y:10});
  gsap.set('#ifsTags',{opacity:0,y:14});
  gsap.set('#ifsCta',{opacity:0,y:14});
  gsap.set('#ifsCard',{opacity:0,y:28,scale:0.98});
  ...
  gsap.set(['#ifsEye','#ifsHL','#ifsBody','#ifsExLabel','#ifsTags','#ifsCta','#ifsCard','#ifsLeg'],{opacity:1,y:0,x:0,scale:1});
}
...
var SX=0.16,FE=0.46,PX=0.60,NP=2000,NO=12;var pR=false;
...
Pt.prototype.init=function(f,li){this.si=Math.floor(Math.random()*3);this.oi=Math.floor(Math.random()*12);this.ci=this.si*12+this.oi;this.v=Math.random();this.col=cF(this.si,this.v);this.bA=0.28+Math.random()*0.52;this.sz=0.5+Math.random()*1.8;this.yO=(Math.random()-0.5)*22;this.wo=Math.random()*Math.PI*2;this.sp=0.9+Math.random()*1.3;this.fS=0.003+Math.random()*0.004;this.rY=(Math.random()-0.5)*4;this.ph=0;this.fT=0;this.y=sY(this.si);this.a=0;if(f&&typeof li==='number'&&li<35){this.x=-(li*5+Math.random()*15);this.sp=1.6+Math.random()*0.9;this.fS=0.006+Math.random()*0.004;}else if(f){this.x=-(Math.random()*W*0.08+2);}else{this.x=-(2+Math.random()*W*0.10);}};
